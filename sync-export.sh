...
files=$(ls /home/ubuntu/export.pid 2> /dev/null | wc -l)
if [ "$files" != "0" ]
then
...
#target_exports=$((cd /home/ubuntu/export/bap; find . -type f -name products.csv -exec dirname {} \;) | sed 's/.//')
...
if [[ ${RUN_ENV} == PRD ]]; then
   ...
   do
      ...
      /usr/local/bin/aws s3 sync /home/ubuntu/export/bap/atero_catalog_epurchasingnetwork_com${export_dir} ${ATERO_S3} --size-only --acl public-read
   done
fi
...
if [[ ${RUN_ENV} == STG ]]; then
   ...
   do
      ...
      /usr/local/bin/aws s3 sync /home/ubuntu/export/bap/staging_catalog_epurchasingnetwork_com${export_dir} ${ATERO_S3} --size-only --acl public-read
   done
fi
