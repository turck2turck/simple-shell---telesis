...
###########################################################################################
#
# Program: su_dealer_testing.sh
# Date:    3/13/2018
# Author:  J.Turck
# User:
#
# Purpose: Stand up a dealer/mfr_rep in testing schema on dev.
#
###########################################################################################
export DEALER=$1
source ${HOME}/config/init.cfg
export PGM_NAME=su_dealer_testing
...
echo "insert into testing.dealer_org (name,logo,message,support_email,is_credit_enabled,credit_period_value,credit_amount,created_at,created_by,dealer_display_name, public_retail_site) 
...
echo "insert into testing.application_user (email, first_name, last_name, email_confirmed, password_hash, user_role, created_at, created_by)
...
echo "insert into testing.dealer (application_user_id,dealer_org_id,is_banned,created_at,created_by)
      select a.id,d.id,'false',current_timestamp,'1'
      from testing.application_user a, public.dealer_org d
...
echo "delete from testing.manufacturer_dealer_assoc 
      where dealer_org_id in (select id from testing.dealer_org where name=${D_NAME}) "> ${SQLDIR}/${SQL_STEP}.sql
psql -h ${HOST} -U ${USER} -d ${DATABASE} -t -v "ON_ERROR_STOP=1" -f ${SQLDIR}/${SQL_STEP}.sql >> ${LOGDIR}/${PGM_NAME}.out 2>>${ELOGDIR}/${PGM_NAME}.err
es=${?}
   if [[ ${es} -ne 0 ]]; then
      echo "Error with the ${SQL_STEP}.sql"
...
do
   echo "insert into testing.manufacturer_dealer_assoc (manufacturer_id,dealer_org_id)
         select m.id, d.id
         from testing.manufacturer m, public.dealer_org d
